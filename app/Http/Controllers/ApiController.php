...
//                UserFcmToken::insertOrIgnore(['user_id' => $auth->id, 'fcm_token' => $request->fcm_id, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now()]);
    ...
    public function addItem(Request $request) {
        try {
            ...
            $data = [
                ...
                'expiry_date' => $user_package->end_date
            ];

            if ($request->hasFile('image')) {
                $data['image'] = FileService::compressAndUploadWithWatermark($request->file('image'), $this->uploadFolder);
            }
    ...
    public function updateItem(Request $request) {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $data['image'] = FileService::compressAndReplaceWithWatermark($request->file('image'), $this->uploadFolder, $item->getRawOriginal('image'));
            }
        ...
        try {
            ...
            $itemOffer = ItemOffer::with(['seller:id,name,profile', 'buyer:id,name,profile', 'item:id,name,description,price,image,status,deleted_at,sold_to', 'item.review' => function ($q) {
                ...
                ->with(['chat' => function ($query) {
                    $query->latest('updated_at')->select('updated_at', 'item_offer_id');
                }])->orderBy('id', 'DESC');
